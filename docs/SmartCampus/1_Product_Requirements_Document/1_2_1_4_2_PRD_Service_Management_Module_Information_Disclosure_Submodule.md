...
5. **归档**
   - **用例描述**：归档过期或取消发布的公告。
   - **前置条件**：公告处于已过期或已取消发布状态，用户拥有archive权限。
   - **输入**：归档操作。
   - **输出**：档案状态的公告。
- **业务规则**：
     - 查询：查看归档公告。
...
- 资源路径遵循命名规范（/api/v1/announcements）
- 查询参数标准化（分页、筛选、排序）
...
**典型API接口示例**：
```
GET /api/v1/announcements?type=1&status=published&page=1&size=20
响应：公告列表数据，包含分页信息
...
PUT /api/v1/announcements/{id}/status
请求体：状态更新数据
响应：更新结果

GET /api/v1/statistics/announcements/read-rate?startDate=2023-01-01&endDate=2023-01-31
响应：指定时间段内公告阅读率统计数据
```
